...
QVariant PluginList::tooltipData(const QModelIndex& modelIndex) const
{
  ...
  if (esp.forceDisabled) {
    ...
    if (feature && esp.hasLightExtension && feature->lightPluginsAreSupported()) {
    ...
    } else if (m_BlueprintPlugins && esp.isBlueprintFlagged && esp.isBlueprintPrefixed) {
      toolTip += "<br><br>" +
      ...
      toolTip += "<br><br>" +
                 tr("This is an invalid blueprint file with no blueprint flag.");
    } else {
...
bool PluginList::setData(const QModelIndex& modIndex, const QVariant& value, int role)
{
  ...
  if (role == Qt::CheckStateRole) {
    ...
    if (m_BlueprintPlugins && !m_ESPs[modIndex.row()].name.startsWith(m_GamePlugin->blueprintPrefix(),
                                                Qt::CaseInsensitive)) {
      QString blueprint = m_GamePlugin->blueprintPrefix();
      blueprint +=
